...
// -------- wizard-done(P4b 终审延期项修复:向导门控迁到后端状态)--------
// 向导是否显示以后端状态为准 —— GET 全新安装必须返回 done:false(不管客户端携带什么
// 陈旧的 localStorage 值,后端从不读取、也不信任客户端提供的"已完成"信号来跳过写入前
// 的默认值),PUT 完成后持久化并可读回。

test('GET /wizard-done 全新安装(全新后端)返回 done:false —— 对应 P4b 回归:重装/工厂重置后即便浏览器本地缓存说已跳过,后端仍以此为准强制再次引导', async () => {
  const { baseUrl, close } = await startApp()
  try {
    const res = await fetch(`${baseUrl}/api/openbox/profile/wizard-done`)
    assert.equal(res.status, 200)
    const body = await res.json()
...
test('PUT /wizard-done {done:true} 后持久化,GET 返回 true', async () => {
  const { baseUrl, store, close } = await startApp()
  try {
    const res = await putJson(baseUrl, '/api/openbox/profile/wizard-done', { done: true })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.done, true)
    assert.equal(store.getWizardDone(), true)

    const getRes = await fetch(`${baseUrl}/api/openbox/profile/wizard-done`)
    const getBody = await getRes.json()
    assert.equal(getBody.done, true)
  } finally {
    await close()
  }
})

test('PUT /wizard-done {done:false} 可重置(供"重新引导"之类的场景使用)', async () => {
  const { baseUrl, store, close } = await startApp()
  try {
    await putJson(baseUrl, '/api/openbox/profile/wizard-done', { done: true })
    const res = await putJson(baseUrl, '/api/openbox/profile/wizard-done', { done: false })
    assert.equal(res.status, 200)
    const body = await res.json()
    assert.equal(body.done, false)
...
test('PUT /wizard-done done 非布尔 → 400 且不写入', async () => {
  const { baseUrl, store, close } = await startApp()
  try {
    const before = store.getWizardDone()
    const res = await putJson(baseUrl, '/api/openbox/profile/wizard-done', { done: 'yes' })
    assert.equal(res.status, 400)
    const body = await res.json()
    assert.ok(body.error)
    assert.equal(store.getWizardDone(), before)
  } finally {
    await close()
  }
})

test('PUT /wizard-done 缺 done 字段 → 400 且不写入', async () => {
  const { baseUrl, store, close } = await startApp()
  try {
    const before = store.getWizardDone()
    const res = await putJson(baseUrl, '/api/openbox/profile/wizard-done', {})
    assert.equal(res.status, 400)
    assert.equal(store.getWizardDone(), before)
  } finally {
    await close()
