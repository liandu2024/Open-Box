// 「共享网络」:在路由器上开几个入站(SS / VLESS / TUIC / Hysteria2),别的设备连上来
// 就经过本机的分流规则上网。档案里存 profile.servers,这里把它们变成 sing-box 入站。
//
...
export const SERVER_PROTOCOLS = ['shadowsocks', 'vless', 'tuic', 'hysteria2']
...
// 防火墙要放行的协议:SS 同时有 TCP/UDP,VLESS 只有 TCP,TUIC/HY2 是 QUIC(UDP)
export const serverFirewallProto = (s) => {
  if (s.protocol === 'shadowsocks') return 'tcp udp'
  if (s.protocol === 'vless') return 'tcp'
  return 'udp'
}
