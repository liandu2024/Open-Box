<template>
  <!-- 选中那天的 24 小时曲线:进站 / 出站两条平滑面积线。数据是内核计数器按采样时刻落到小时桶里的
       ...
       数值不藏在悬停提示里:图下面那行图例常驻写着某个小时的进站 / 出站——默认是峰值那个小时,
       鼠标(或手指)在图上移到哪个小时就换成哪个小时,离开又回到峰值。 -->
...
const render = (force = false) => {
  ...
  const series = (key: 'down' | 'up', color: string) => ({
    type: 'line',
    smooth: true,
    showSymbol: false,
  ...
  chart.setOption({
    ...
    series: [series('down', c.inbound), series('up', c.outbound)],
  })
