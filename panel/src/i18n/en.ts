const en = {
  ...
  dnsModeOffNote: 'The kernel rewrites no DNS and auto_redirect stays off; resolution is up to the LAN\'s existing DNS server. To use the kernel\'s split resolution, point AdGuard Home / Pi-hole upstream at the kernel DNS address below.',
  dnsModeUpstreamHint: 'Kernel DNS address: {addr} (available while the kernel runs, in every mode). Point a LAN AdGuard Home / Pi-hole upstream at it to use the kernel\'s split resolution.',
