const en = {
  ...
  subscriptionUrlHint: 'Required to save; the server fetches it (private / local addresses are rejected). The https certificate is not verified, so self-signed servers work too.',
  ...
  icmpTitle: 'Always ping directly (ICMP)',
  icmpDescription: 'Off by default. When off, pings follow the same routing as other traffic: direct destinations (CN sites, node servers) return the real latency, proxied destinations are answered locally by the kernel; restart the kernel to apply.',
  icmpOnNote: 'On: every ping leaves the router directly and returns the direct-path latency; blocked targets time out and look unreachable. No proxy protocol can carry ICMP, so pinging through a node is not possible.',
  icmpOffNote: 'Off: direct destinations show real latency; proxied destinations are answered by the kernel — a reply means reachable, but the ~1 ms is not a latency. Use the panel latency test for latency through a node.',
  ipv6Title: 'IPv6',
  ...
  dnsModeOffNote: 'The kernel leaves DNS alone and auto_redirect is disabled; resolution depends entirely on the LAN\'s existing DNS server, and routing relies on sniffed domains and IP rules. Proxied sites may fail if the upstream DNS is polluted.',
