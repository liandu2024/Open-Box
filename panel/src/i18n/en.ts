const en = {
  ...
  failoverStaticHint: 'Static members · pick nodes on the left and add them to the current lane on the right. The first lane is primary, the rest are backups in order; a lane with one valid node is used directly, several valid nodes form an internal url-test subgroup.',
  ...
  failoverLaneCountHint: 'Lanes passing the probe / total lanes',
  failoverNoLane: 'no usable lane',
  failoverNoStatus: 'not probed yet',
  failoverPausedKernel: 'kernel API unreachable, probing paused',
  failoverPausedMismatch: 'kernel outbounds do not match the current definition (deploying?), round skipped',
  failoverLastRound: 'last probe {time}',
  failoverLastSwitch: 'last switch: {from} → {to} ({reason}) · {time}',
  failoverCurrent: 'current',
  failoverKernelNow: 'kernel pick',
  failoverProbeFailed: 'failed',
  failoverProbeUnknown: 'not probed',
  failoverLaneEmpty: 'this lane has no members',
  failoverHealthUp: 'up',
  failoverHealthDown: 'down',
  failoverHealthUnknown: 'unknown',
  failoverStatusPending: 'probing',
  failoverStatusOk: 'primary',
  failoverStatusBackup: 'on backup',
  failoverStatusFailing: 'current lane failing',
  failoverStatusReject: 'all down',
  failoverStatusUnknown: 'unconfirmed',
