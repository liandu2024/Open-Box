const en = {
  ...
  diagnosticsDescription: 'For bug reports: versions, firmware, architecture, kernel state, redacted config and recent logs in one file you can drop into a GitHub issue.',
  ...
  backupDescription: 'Bundle everything configured on this router into one file: routing, site sets, node groups, DNS, update plans, panel settings (with the background image) and, optionally, subscriptions and nodes, client routes and shared network. Import it on a new device and skip the setup.',
  ...
  dnsModeOff: 'Off',
  dnsModeDnsmasqNote: 'The router\'s dnsmasq stays the LAN resolver; only its upstream is pointed at the kernel (restored when the kernel stops). Local hostnames and DHCP names keep working. Recommended.',
  dnsModeHijackNote: 'The firewall rewrites every LAN query to port 53 straight to the kernel, bypassing dnsmasq; local hostnames are handed back to dnsmasq by the kernel.',
  dnsModeOffNote: 'The kernel rewrites no DNS and auto_redirect stays off; resolution is up to the LAN\'s existing DNS server. The router\'s own dnsmasq then uses the ISP resolver, whose answers for foreign domains are polluted — point clients at AdGuard Home / Pi-hole (not the router\'s port 53) and point those at the kernel DNS address below.',
  ...
  dnsFakeIpNote: 'Controlled experiment only, production use undecided, off by default. When on, domains matched by a proxied policy are not resolved locally: the kernel first answers with a placeholder address (198.18.0.0/15); when the client connects, the kernel maps the placeholder back to the domain and hands it to the currently selected outbound. Only outbounds that can carry a domain (SOCKS5 / vless / vmess / trojan / hysteria …) let the node resolve it; WireGuard and SOCKS4 resolve inside the kernel first. When rules that match by destination IP come first (reject / forced outbound / GeoIP), the kernel resolves the domain with that policy\'s own resolver before evaluating the rules in order, and those connections reach the node as an IP rather than a domain; only without such rules is the domain handed to the node. Clients with their own encrypted DNS, real addresses cached before enabling, and queries in DNS-off mode never go through placeholders. Restart the kernel after switching; after turning it off, placeholders cached on clients last until their TTL expires.',
  ...
  routeTestDomainTargetNote: 'This probe entered the kernel by domain through the loopback inbound, unlike the tun path where a client connects to the IP it resolved: when an IP-based rule precedes the domain rules, the kernel first resolves the domain with that policy\'s resolver and then evaluates the rules in order; otherwise it matches by domain. A client\'s real access follows the address the client obtained.',
  serverDomainTargetNote: 'Connections made by domain through these SOCKS5 / HTTP proxies reach the kernel with a domain target rather than an IP. When rules that match by destination IP come first (custom IP rows, GeoIP / IP ranges in policies), the kernel resolves the domain with that policy\'s resolver first and then evaluates the rules in order; without such rules it matches by domain. This differs from the tun path, where the client resolves first and connects by IP.',
