...
const zh: LANG_MESSAGE = {
  ...
  dnsModeOffNote: '内核不改写任何 DNS,也不开 auto_redirect;解析全看局域网现有的 DNS 服务器。想用内核的分流解析,把 AdGuard Home / Pi-hole 的上游指向下面的内核 DNS 地址。',
  dnsModeUpstreamHint: '内核 DNS 地址:{addr}(内核运行时可用,三种模式相同)。局域网里的 AdGuard Home / Pi-hole 把上游指向它,就能用内核的分流解析;不要指向 53 端口以外的其它端口。',
