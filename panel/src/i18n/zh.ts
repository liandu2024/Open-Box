...
const zh: LANG_MESSAGE = {
  ...
  icmpTitle: 'Ping(ICMP)一律直连',
  icmpDescription: '默认关闭。关着时 ping 和别的流量一样按分流走:直连目标(国内站、节点服务器)返回真实延迟,走代理的目标由内核本地代答;重启内核生效。',
  icmpOnNote: '开启中:所有 ping 都从路由器直连发出,返回的是本地直连到目标的真实延迟;被墙的目标会超时,看起来像打不开。没有代理协议能承载 ICMP,做不到经节点 ping。',
  icmpOffNote: '关闭中:直连目标是真实延迟;走代理的目标由内核代答,ping 通表示能访问,但那个 1ms 不是延迟——想看经代理的延迟用面板的延迟测试。',
  ipv6Title: 'IPv6',
  dnsModeTitle: 'DNS 劫持',
  ...
  dnsModeOffNote: '内核完全不碰 DNS,也不开 auto_redirect;解析全看局域网现有的 DNS 服务器,分流只靠嗅探到的域名和 IP 规则。上游 DNS 不干净时走代理的站点可能打不开。',
