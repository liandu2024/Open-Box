...
const zh: LANG_MESSAGE = {
  ...
  proxiesPendingSiteSets: '站点集「{names}」已保存,但内核还在用旧配置跑,重启内核后才会出现在这里。',
  proxiesPendingApply: '重启内核并应用',
  proxiesPendingApplyFailed: '重启内核失败:{detail}',
  proxiesPendingApplied: '已应用,内核重启完成。',
  upgradeUI: '更新面板',
  reloadConfigs: '重载配置',
  mode: '模式',
  ...
  routingPoliciesDescription: '一个站点集 = 一组匹配规则。命中的流量走这个站点集选中的线路(在「代理」页选)。按顺序匹配,可以拖拽调整;都没命中的走最下面的兜底。改动保存后要重启内核才生效(代理页会提示并可一键重启)。',
