...
const zhTW: LANG_MESSAGE = {
  ...
  icmpTitle: 'Ping(ICMP)一律直連',
  icmpDescription: '預設關閉。關著時 ping 和別的流量一樣按分流走:直連目標(國內站、節點伺服器)回傳真實延遲,走代理的目標由核心本機代答;重啟核心生效。',
  icmpOnNote: '開啟中:所有 ping 都從路由器直連發出,回傳的是本機直連到目標的真實延遲;被牆的目標會逾時,看起來像打不開。沒有代理協定能承載 ICMP,做不到經節點 ping。',
  icmpOffNote: '關閉中:直連目標是真實延遲;走代理的目標由核心代答,ping 通表示能存取,但那個 1ms 不是延遲——想看經代理的延遲用面板的延遲測試。',
  ipv6Title: 'IPv6',
  dnsModeTitle: 'DNS 劫持',
  ...
  dnsModeOffNote: '核心完全不碰 DNS,也不開 auto_redirect;解析全看區域網路現有的 DNS 伺服器,分流只靠嗅探到的網域和 IP 規則。上游 DNS 不乾淨時走代理的站點可能打不開。',
