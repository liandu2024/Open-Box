...
const zhTW: LANG_MESSAGE = {
  ...
  proxiesPendingSiteSets: '站點集「{names}」已儲存,但核心還在用舊設定跑,重啟核心後才會出現在這裡。',
  proxiesPendingApply: '重啟核心並套用',
  proxiesPendingApplyFailed: '重啟核心失敗:{detail}',
  proxiesPendingApplied: '已套用,核心重啟完成。',
  upgradeUI: '更新面板',
  reloadConfigs: '重載配置',
  mode: '模式',
  ...
  routingPoliciesDescription: '一個站點集 = 一組比對規則。命中的流量走這個站點集選中的線路(在「代理」頁選)。按順序比對,可以拖曳調整;都沒命中的走最下面的兜底。變更儲存後要重啟核心才生效(代理頁會提示並可一鍵重啟)。',
