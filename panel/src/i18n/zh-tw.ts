...
const zhTW: LANG_MESSAGE = {
  ...
  dnsModeOffNote: '核心不改寫任何 DNS,也不開 auto_redirect;解析全看區域網路現有的 DNS 伺服器。想用核心的分流解析,把 AdGuard Home / Pi-hole 的上游指向下面的核心 DNS 位址。',
  dnsModeUpstreamHint: '核心 DNS 位址:{addr}(核心執行時可用,三種模式相同)。區域網路裡的 AdGuard Home / Pi-hole 把上游指向它,就能用核心的分流解析。',
