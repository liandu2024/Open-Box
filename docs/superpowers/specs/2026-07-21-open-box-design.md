...
不用 fake-ip 时,DNS 返回真实 IP,**DNS 解析质量直接决定分流正确性**。核心原则:**直连 DNS 不经代理节点,代理 DNS 必须经代理节点出站**,双通道彻底隔离,防污染。

**两组 DNS 服务器**:

| 通道 | 出厂默认 | 出站方式 | 用途 |
| --- | --- | --- | --- |
| 直连 DNS(dns_direct) | 223.5.5.5(备 119.29.29.29) | direct 出站,不经代理 | 国内域名,追求低延迟与 CDN 就近解析 |
| 代理 DNS(dns_proxy) | https://1.1.1.1/dns-query(备 https://8.8.8.8/dns-query) | **detour 经代理策略组出站** | 国外域名,加密 + 代理链路双重防污染 |

两组服务器地址均可在面板设置中修改;代理 DNS 默认用 IP 形态的 DoH 地址,**规避"解析 DoH 域名"的自举死锁**(用户改成域名形态时,该域名强制走直连 DNS 解析)。

**分流规则(按优先级)**:

1. 代理节点服务器域名 → 直连 DNS(经 `route.default_domain_resolver` 指向 dns_direct,避免"解析节点域名需要代理、代理又需要先解析节点域名"的循环依赖)
2. 命中国内域名规则集(geosite-cn 类,本地 .srs)→ 直连 DNS
3. 用户自定义域名规则(面板可视化编辑,可指定走直连/代理 DNS)
4. **其余一切(含未知域名)→ 代理 DNS**——兜底方向朝代理,宁可国内冷门域名慢一点,不可国外域名被污染

**与路由分流的一致性约束**:DNS 判定与路由判定必须使用同一套域名规则集——被判为国内的域名,DNS 走直连、连接也走 direct;被判为国外的域名,DNS 走代理、连接也走代理。路由侧对 TCP/UDP 连接开启 sniff,优先按嗅探出的域名匹配规则,而不是按解析后的 IP,避免"国外域名解析到 CDN 国内节点导致错误直连"这类经典错位。
...
### 4.4 直连判定规则(域名与 IP 的数据源和判定链)

**数据源**(全部本地化为 .srs,由面板调度下载更新,见 6.3):

| 规则集 | 来源 | 内容 |
| --- | --- | --- |
| geosite-cn | SagerNet/sing-geosite 官方发布(数据来自 v2fly domain-list-community,含 dnsmasq-china-list) | 国内网站/CDN/大厂域名 |
| geoip-cn | SagerNet/sing-geoip 官方发布 | 中国大陆 IP 段 |
...
4. 域名规则:sniff 嗅探出的域名命中 geosite-cn → 直连
5. IP 规则:目标 IP 命中 geoip-cn → 直连(兜底给嗅探不出域名的裸 IP 连接,如游戏/P2P)
6. 其余一切 → 代理策略组

域名规则(4)排在 IP 规则(5)之前:"国外域名解析到国内 CDN IP"的连接在第 4 步已按域名判定走代理,不会被 geoip-cn 错误拦下直连;纯 IP 流量则由 geoip-cn 兜底。

DNS 侧判定链(4.3 节)与此使用同一套 geosite-cn 与用户自定义清单,保证 DNS 与路由判定一致。
