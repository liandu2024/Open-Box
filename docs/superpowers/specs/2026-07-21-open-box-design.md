...
- 订阅导入后,按节点名称/区域标识**自动生成区域节点组**(美国、新加坡、日本、香港…),组类型支持手动选择(select)与自动测速(urltest)
...
2. 面板侧:单后端裁剪、首次引导向导(语言/区域/推荐默认/订阅)、订阅解析(三格式七协议)、策略分流模板 + 策略组可视化编辑、DNS 分流开关、规则集本地化、本地下发与服务控制、clash_api 中转、自动更新调度、一键升级、登录认证、三语言(英/简/繁)与三主题(系统/亮/暗)
3. 文档:README(两条安装命令、硬件要求、常见问题)、公网访问安全警告

## 12. 测试策略

- **面板后端**:订阅解析(各格式/协议样本)、配置生成(策略分流装配 × DNS 分流开关 × DNS 两接管模式 × IPv6 开关)单元测试;`sing-box check` 作为配置生成的金标准集成测试
