...
Open-Box 面向全球用户,**不以"国内/国外"为分流概念**。分流模型只有两个走向:**直连**与**代理**(广告拦截可选叠加"拒绝")。整套分流配置就三要素:

1. **直连侧规则集**:哪些区域/域名/IP 走直连
2. **代理侧规则集**:哪些区域/域名/IP 走代理
3. **兜底方向**:两边都没命中的未知流量走直连还是代理

用户在首次引导中选择**本地区域**(CN、US、JP…),系统据此生成**默认装配**:本地区域规则集 → 直连侧,兜底 → 代理。之后用户可在可视化编辑里把任意区域/规则集移到另一侧、翻转兜底方向。两类典型人群都是同一模型的不同装配:

- 身处受限网络(典型 CN 用户):本地区域 → 直连,兜底 → 代理(默认装配即是)
- 海外用户访问受限区域服务(如中国网站/AI):目标区域(如 CN)→ 代理侧,兜底改为直连

首次引导页完成:语言(自动检测可改)→ 本地区域 → 兜底方向确认(按区域给推荐)→ 订阅导入。

### 4.4 DNS 分流

不用 fake-ip 时,DNS 返回真实 IP,**DNS 解析质量直接决定分流正确性**。核心原则:**直连 DNS 不经代理节点,代理 DNS 必须经代理节点出站**,双通道彻底隔离——既防污染,也让代理侧域名解析到目标区域内的正确 CDN 节点(地理就近解析)。
...
| 代理 DNS(dns_proxy) | **detour 经代理策略组出站** | 默认 https://1.1.1.1/dns-query(IP 形态 DoH);把 CN 规则集放到代理侧的用户可改为经代理的 223.5.5.5 这类目标区域 DNS |
...
3. 命中直连侧域名规则集 → 直连 DNS;命中代理侧域名规则集 → 代理 DNS(装配见 4.5)
4. 其余未知域名 → 跟随兜底方向(兜底代理 → 代理 DNS;兜底直连 → 直连 DNS)
...
**规则集装配**:默认装配随首次引导自动完成(本地区域规则集 → 直连侧,兜底 → 代理),之后用户可在可视化编辑里把任意区域/规则集放到直连侧或代理侧,并切换兜底方向(见 4.3)。
...
- 内置分流模板按"直连/代理/兜底"三要素装配(见 4.3/4.5),默认装配随本地区域生成,广告拦截可选叠加;用户无需理解 geosite/geoip 概念
- 首次引导向导:语言 → 本地区域 → 兜底方向确认 → 订阅导入,走完即可用
- 策略组、规则集可视化增删改(含把任意规则集在直连侧/代理侧之间移动)
...
2. 面板侧:单后端裁剪、首次引导向导(语言/区域/兜底方向/订阅)、订阅解析(三格式七协议)、直连/代理装配模板 + 可视化编辑、规则集本地化、本地下发与服务控制、clash_api 中转、自动更新调度、一键升级、登录认证、三语言(英/简/繁)与三主题(系统/亮/暗)
3. 文档:README(两条安装命令、硬件要求、常见问题)、公网访问安全警告

## 12. 测试策略

- **面板后端**:订阅解析(各格式/协议样本)、配置生成(模板合并、直连/代理装配 × 兜底方向、DNS 两接管模式、IPv6 开关)单元测试;`sing-box check` 作为配置生成的金标准集成测试
