...
- **目标分流**:站点集按域名 / 域名后缀 / 关键词 / 规则集 / 规则集链接 / IP 段匹配,出口在代理页点选,选完即成为默认。规则集来自 MetaCubeX/meta-rules-dat 的 sing 分支,geosite 1899 类、geoip 260 类,可按需下载;规则集链接则可以直接填一个现成的名单网址,Clash 的 `.list` 和 mihomo 的 `.mrs` 都认,填完就显示条数、点开能看内容。
